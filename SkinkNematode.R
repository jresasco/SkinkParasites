...
# subset for with and without matrix
DataTreatSub <- subset(Data, TreatNum < 2.5) # for models including the matrix make this "3.5"; for analyses without the matrix use "2.5"
head(DataTreatSub)


###Make factors # make size, edge, topo factors
DataTreatSub$TreatNum <- factor(DataTreatSub$TreatNum, levels=c(2,1,3))
DataTreatSub$Rep <- factor(DataTreatSub$Rep)
DataTreatSub$SizeNum <- factor(DataTreatSub$SizeNum)
DataTreatSub$ProxNum <- factor(DataTreatSub$ProxNum)
DataTreatSub$TopoNum <- factor(DataTreatSub$TopoNum)
...
# Hedruris models
#GLMMs for Hedruris wogwogensis gut prevalance and intenisty without Matrix (set to 2.5)
# Null models 
m0 <- glm(HWPres ~  1 , data=DataTreatSub, family=binomial) 
m0i <- glm(HWCount ~  1 , data=DataTreatSub, family=poisson)

# Treat model 
m1  <- glm(HWPres ~ TreatNum , data=DataTreatSub, family=binomial) 
m1i  <- glm(HWCount ~ TreatNum , data=DataTreatSub, family=poisson) 
...
#confint(m1) # CIs used
anova(m0i,m1i) 
PBmodcomp(m1i, m0i, nsim = 100, ref = NULL, seed=NULL,
          cl = NULL, details = 0)
summary(m1i)
#confint(m1) # CIs used

tapply(DataTreatSub$HWCount,DataTreatSub$SizeNum, mean)

# Size models
m2 <- glm(HWPres ~ TreatNum + SizeNum , data=DataTreatSub, family=binomial) 
m2i <- glm(HWCount ~ TreatNum + SizeNum , data=DataTreatSub, family=poisson) 
...
summary(m2)
anova(m1i,m2i) 
PBmodcomp(m2i, m1i, nsim = 100, ref = NULL, seed=NULL,
          cl = NULL, details = 0)
summary(m2i)


# topo model
m3 <- glm(HWPres ~ TreatNum + TopoNum , data=DataTreatSub, family=binomial) 
m3i <- glm(HWCount ~ TreatNum + TopoNum, data=DataTreatSub, family=poisson) 
...
anova(m1i,m3i) 
PBmodcomp(m3i, m1i, nsim = 100, ref = NULL, seed=NULL,
          cl = NULL, details = 0)
summary(m3i)

tapply(DataTreatSub$HWCount,DataTreatSub$ProxNum, mean)

# edge model
m4 <- glm(HWPres ~ TreatNum + ProxNum  , data=DataTreatSub, family=binomial)
m4i <- glm(HWCount ~ TreatNum + ProxNum, data=DataTreatSub, family=poisson) 
...
anova(m1i,m4i)
PBmodcomp(m4i, m1i, nsim = 100, ref = NULL, seed=NULL,
          cl = NULL, details = 0)
summary(m4i)
#confint(m4)
tapply(DataTreatSub$HWCount,DataTreatSub$ProxNum, mean)
...
# test Treat vs Null
anova(m0,m1) #no bias correction
anova(m0BC,m1BC) # bias correction
summary(m1)
summary(m1BC)
PBmodcomp(m1BC, m0BC, nsim = 100, ref = NULL, seed=NULL,
          cl = NULL, details = 0)
